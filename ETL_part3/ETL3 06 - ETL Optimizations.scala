...
val uncompressedPath = s"$workingDir/academy/pageCountsUncompressed.csv"
val snappyPath =       s"$workingDir/academy/pageCountsSnappy.csv"
val gzipPath =         s"$workingDir/academy/pageCountsGZIP.csv"
...
println(s"Snappy:       $snappySize bytes")

val GZIPSize = dbutils.fs.ls(gzipPath).map(x => x.size).sum
println(s"GZIP:         $GZIPSize bytes")
...
def timeIt(df: Dataset[Row], fileType: String, partitions: Int = 1, compression: String="", 
           outputPath: String = s"$workingDir/comparisonTest"): Unit = {
