...
val cleanDF = initialDF
 .select((col("value").cast("STRING")))  // Select the "value" column, cast "value" column to STRING
 ...
 .withColumn("epoc", unix_timestamp(col("ts_string"), "yyyy/MM/dd HH:mm:ss.SSS"))  // Select the "ts_string" column, apply unix_timestamp to it and rename to "epoc"
 .withColumn("capturedAt", col("epoc").cast("timestamp"))  // Select the "epoc" column and cast to a timestamp and rename it to "capturedAt"
 .withColumn("logData", regexp_extract(col("value"), """^.*\]\s+(.*)$""", 1))  // Select the "logData" column and apply the regexp `"""^.*\]\s+(.*)$"""`
...
val ipDF = cleanDF
 ...
 .withColumn("ipClass" ,when(col("ip") < 127, "Class A")// figure out class of IP address based on first two octets                          
