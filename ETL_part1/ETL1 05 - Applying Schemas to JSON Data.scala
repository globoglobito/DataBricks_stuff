...
// TODO
smartphoneDF.schema.foreach(println)
...
// TODO
val SMSDF = spark.read.schema(schema).json("/mnt/training/UbiqLog4UCI/14_F/log*").na.drop() // option("mode", "DROPMALFORMED") didnt drop nulls even if the schema is defined as nullable
...
val schema2 = StructType(List(
  StructField("SMS", 
              StructType(List(
                ...
                StructField("metadate",StructType(List(
...
val SMSDF2 = spark.read
             .schema(schema2)
             .json("/mnt/training/UbiqLog4UCI/14_F/log*").where(col("SMS").isNotNull)

SMSDF2.show(100, false)
...
dbTest("ET1-S-05-04-01", 1147, SMSDF2.count)
