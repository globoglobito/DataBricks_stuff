...
val dfMin = peopleDF.select(min(col("id"))).first().getInt(0) // getInt is needed to get the actual value and not just the row (even if the row itself is one value)
val dfMax = peopleDF.select(max(col("id"))).first().getInt(0) // we use getInt instead of get, because otherwise the .asInstance fails
...
// TODO
val peopleDFParallel = (spark.read.jdbc(url=jdbcUrl,
  table=tableName,
  columnName="""id""",
  lowerBound=1L,
  upperBound=100000L,
